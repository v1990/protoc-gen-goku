...
func (c *postsService) Report(ctx context.Context, in *ReportRequest, opts ...client.CallOption) (*empty.Empty, error) {
	req := c.c.NewRequest(c.name, "Posts.Report", in)
	out := new(empty.Empty)
	err := c.c.Call(ctx, req, out, opts...)
	if err != nil {
		return nil, err
...
func RegisterPostsHandler(s server.Server, hdlr PostsHandler, opts ...server.HandlerOption) error {
	type posts interface {
		...
		Report(ctx context.Context, in *ReportRequest, out *empty.Empty) error
	}
	type Posts struct {
		posts
	}
...
func (h *postsHandler) Report(ctx context.Context, in *ReportRequest, out *empty.Empty) error {
	return h.PostsHandler.Report(ctx, in, out)
}
